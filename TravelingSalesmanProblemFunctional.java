...
public class TravelingSalesmanProblemFunctional
{
    ...
    private void solveTravelingSalesmanProblem() {
        ...
        final Deque<Edge> shortestPath = findShortestPath(memo, optimalPathData.getKey());
    ...
    private Deque<Edge> findShortestPath(final Map<Integer, Map<Path, Integer>> memo, final Path optimalPath) {
        final Deque<Edge> shortestPath = new ArrayDeque<>();

        shortestPath.add(new Edge(optimalPath.suffixNodeID(), S));
        shortestPath.addFirst(new Edge(-1, optimalPath.suffixNodeID()));

        return findPreviousNodeAlongShortestPath(shortestPath, optimalPath, nodesCount - 1, memo).trigger();
    }

    private Tailcall<Deque<Edge>> findPreviousNodeAlongShortestPath(final Deque<Edge> shortestPath, final Path optimalPathSMCN,
            final int nodesCountInLatestPathSMC, final Map<Integer, Map<Path, Integer>> memo) {

        if (shortestPath.size() == nodesCount) {
            final Edge latestEdge = shortestPath.pollFirst();
            shortestPath.addFirst(new Edge(S, latestEdge.endNodeID()));
            shortestPath.addFirst(new Edge(S, S));
    ...
    private Tailcall<Deque<Edge>> continueTailcalling(final Deque<Edge> shortestPath, final Path optimalPathSMCN,
            final int nodesCountInLatestPathSMC, final Map<Integer, Map<Path, Integer>> memo) {
        ...
        final Edge latestEdge = shortestPath.pollFirst();
        shortestPath.addFirst(new Edge(previousOptimalPathSMC.suffixNodeID(), latestEdge.endNodeID()));
        shortestPath.addFirst(new Edge(-1, previousOptimalPathSMC.suffixNodeID()));
    ...
    private void printResult(final int optimalCost, final Deque<Edge> shortestPath) {
        System.out.println();
    ...
    private void printShortestPath(final Deque<Edge> shortestPath) {
    ...
    private void printShortestPathNodeIDs(final Deque<Edge> shortestPath) {
        final String printableString = shortestPath.stream()
            .map(edge -> String.format("%3d ", edge.endNodeID()))
            .reduce("", String::concat);

        System.out.println(printableString);
    }

    private void printShortestPathNodeLiterals(final Deque<Edge> shortestPath) {
        final String printableString = shortestPath.stream()
            .map(edge -> String.format("%3c ", edge.endNodeID() + 'A'))
            .reduce("", String::concat);

        System.out.println(printableString);
    }

    private void printShortestPathEdgeWeights(final Deque<Edge> shortestPath) {
        final String printableString = shortestPath.stream()
            .map(graph::get)
            .map(cost -> String.format("%3d ", cost))
            .reduce("", String::concat);

        System.out.println(printableString);
    }

    private void printShortestPathCumulativeEdgeWeights(final Deque<Edge> shortestPath) {
        final List<Integer> edgeWeights = shortestPath.stream()
            .map(graph::get)
            .collect(Collectors.toList());

        final String printableString = IntStream.range(0, edgeWeights.size())
            .map(weightID -> IntStream.rangeClosed(0, weightID)
                .map(edgeWeights::get)
                .sum())
            .mapToObj(cumulativeEdgeWeight -> String.format("%3d ", cumulativeEdgeWeight))
            .reduce("", String::concat);
